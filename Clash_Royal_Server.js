const express = require('express');
const app = express();
const https =  require("https");

const hostname = '127.0.0.1';
const port = 8080;



const set_headers = (req, res, next) => {
    res.setHeader('Content-Type','application/json'); 
    res.setHeader('Access-Control-Allow-Origin', "*");
    res.setHeader('Access-Control-Request-Headers', '*');
    res.setHeader('Access-Control-Allow-Headers', '*');
    res.setHeader('Access-Control-Allow-Methods', "OPTIONS, POST, GET");
    if ('OPTIONS' === req.method) {
        res.sendStatus(200);
        }
        else {
          next();
        }
}

const varify_user_id = (req, res, next) => {
    if(String(req.body.player_id).length != 9){
        // This needs to handle errors.
    }
    else{
        console.log(req.body.player_id)
        console.log(req.body.start_time)
        console.log(req.body.last_refresh_time)
        next();
    }
}

const call_api = (req, c, next) => {
    let token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..AEC19A-Gc-CxCEHWs5d-qr9F75GzKf9MtKkIgoSNv_MKWGtmL3Py6RDq8OfHe_eycMcKPJ_rZpn0mYZZsEz3nQ";
    const api_url_begin = "api.clashroyale.com";
    const api_url_end = "/v1/players/%23" + req.body.player_id  + "/battlelog";
    
    console.log(api_url_begin);
    console.log(api_url_end);
    
    const options = {
      hostname: api_url_begin,
      path: api_url_end,
      method: 'GET',
      headers: {
        Authorization: token,
        ContentType: 'application/json'
    }
    };
    
    https.get(options, resp => {
        let data = "";
        // A chunk of data has been recieved.
        resp.on("data", chunk => {
          data += chunk;
        });

        // The whole response has been received. Print out the result.
        resp.on("end", () => {
          //let data1 = data.slice(1, -1);
          req.body['Clash_Royal_Data'] = JSON.parse(data); //changed from JSON.parse(data)
            next();
        });
      })
      .on("error", err => {
        console.log("Error: " + err.message);
      });
    //IT WORKS IT WORKS IT WORKS!!!!!
} // End of the call_api function.

const extract_data = (req, res, next) => {
    let win_total = 0;
    let game_total = 0;
    let start_time = req.body.start_time;
    let last_refresh_time = req.body.last_refresh_time;
    let current_record = req.body.Clash_Royal_Data[2];
    
    if((start_time < current_record.battleTime) && (last_refresh_time < current_record.battleTime)){
        console.log(current_record.battleTime)};
    
//    for (let i=0; i< 25; i++;){
//        let current_record = req.body.Clash_Royal_Data[i];
//        if((start_time < current_record.battleTime) && (last_refresh_time < current_record.battleTime)){
//            game_total += 1;
//            if(current_record.team.crowns > current_record.opponent.crowns){
//                win_total += 1
//            }//end of win in
//        }//end of time match if statment
//    }//end of the for loop
    next();
}

const function2 = (req, res, next) => {
    console.log("function 2");
    res.send('Hello World!')
}


//This is the order the functions will fire in.
//app.use(bodyParser.json());
app.use(express.json())
app.use(set_headers)
app.use(varify_user_id)
app.use(call_api)
app.use(extract_data)
app.use(function2)

app.post('/', (req, res) => {
    //console.log(res.json({requestBody: req.body}))
    });

app.listen(port, () => console.log('Server ready'));


